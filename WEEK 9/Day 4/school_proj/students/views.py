...
def search(request):
    colors = [ "green", "pink", "cyan", "orange", "purple"]
    
    sleep(3)
    
    data = json.loads(request.body)
   
    result = Student.objects.filter(first_name__icontains=data['searchterm'])
    ...
    response = json.dumps({'info':students, 'color':choice(colors)})
